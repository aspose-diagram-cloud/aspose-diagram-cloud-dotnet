...
namespace Aspose.Diagram.Cloud.Live.Demos.UI.Config
{
    ...
    public class GlobalAppHelper
    {
		...
		public GlobalAppHelper(HttpContext hc, HttpApplicationState appState, string SessionID, string language)
		{
			...
			AsposeDiagramCloudContext.atcc = new AsposeDiagramCloudContext(hc); // sync the context
		...
		private void initResources(string ResourcesFile, string SessionID)
		{
			...
			if (AsposeDiagramCloudContext.Current.Cache[SessionID] == null)
			{
				...
				AsposeDiagramCloudContext.Current.Cache.Add(
				   SessionID,
					resources,
